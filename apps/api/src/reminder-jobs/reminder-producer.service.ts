  ...
  async checkAndEnqueueReminders() {
    ...
    try {
      ...
      for (const reminder of reminders) {
        ...
        // Simple timezone calculation (hours offset)
        // For MVP v1, use simplified approach
        const tzOffsets = {
          'Asia/Bangkok': 7,
          'Asia/Ho_Chi_Minh': 7,
          'UTC': 0,
        ...
        const offset = tzOffsets[userTz] || 7;
        const utcHour = now.getUTCHours();
        const currentHourInTz = (utcHour + offset + 24) % 24;
        const currentMinute = now.getUTCMinutes();

        // Check if current hour matches reminder hour (allow 5-minute window)
        if (currentHourInTz === reminder.hour && currentMinute < 5) {
          this.logger.log(
            `✓ Enqueueing reminder for ${reminder.user.email} (${reminder.type} at ${reminder.hour}:00 ${userTz})`,
          );

          const timestamp = now.toISOString().substring(0, 13); // YYYY-MM-DDTHH
