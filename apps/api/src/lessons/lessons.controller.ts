...
export class LessonsController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Share lesson to public community feed' })
  async shareToCommunity(@Req() req, @Param('id') id: string) {
    return this.lessonsService.shareToCommunity(id, req.user.userId);
  }

  @Post(':id/share-to-group')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Share lesson to a group' })
  async shareToGroup(
    @Req() req,
    @Param('id') id: string,
    @Body('groupId') groupId: string,
  ) {
    return this.lessonsService.shareToGroup(id, req.user.userId, groupId);
  }

  @Get(':id/share-url')
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get shareable URL for lesson' })
  async getShareUrl(@Req() req, @Param('id') id: string) {
    return this.lessonsService.getShareUrl(id, req.user.userId);
  }
