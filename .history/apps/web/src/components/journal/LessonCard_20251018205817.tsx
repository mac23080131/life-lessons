...
import { Edit, Trash2, ExternalLink, Sparkles, Share2 } from 'lucide-react';
import { getMoodEmoji, getDomainColor } from '@/lib/utils';
import { useState } from 'react';
import { ShareModal } from '../lessons/ShareModal';
...
export function LessonCard({ lesson, index, onEdit, onDelete }: LessonCardProps) {
  ...
  return (
    <motion.div
      ...
      layout
    className="group glass-card p-6 rounded-2xl hover:shadow-xl transition-all duration-300 cursor-pointer relative"
  ...
  <div className="flex items-center gap-1 opacity-0 md:opacity-0 md:group-hover:opacity-100 sm:opacity-100 transition-opacity duration-200">
          <button
            onClick={(e) => {
              e.stopPropagation();
