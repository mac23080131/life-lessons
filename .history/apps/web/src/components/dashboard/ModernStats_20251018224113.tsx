...
export function StatsCard({ title, value, change, icon, gradient, delay = 0 }: StatsCardProps) {
  return (
    <motion.div
      ...
      className={cn(
        'glass-card p-4 sm:p-6 rounded-2xl relative overflow-hidden group',
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="glass-card p-4 sm:p-6 rounded-2xl h-28 sm:h-32 animate-pulse">
            <div className="h-10 w-10 sm:h-12 sm:w-12 bg-muted rounded-xl mb-3 sm:mb-4" />
