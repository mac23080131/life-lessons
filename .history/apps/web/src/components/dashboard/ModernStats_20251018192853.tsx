...
export function ModernDashboardStats() {
  ...
  const stats = useMemo(() => {
    ...
    if (total > 0) {
      const oldestLesson = new Date(Math.min(...lessons.map(l => new Date(l.createdAt).getTime())));
      ...
      const last30Count = lessons.filter(l => new Date(l.createdAt) >= last30Days).length;
      const prev30Count = lessons.filter(l => {
      ...
      // Domain breakdown
      const domainCounts = lessons.reduce((acc, lesson) => {
