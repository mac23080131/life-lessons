...
export function ModernDashboardStats() {
  ...
  const stats = useMemo(() => {
    ...
    const thisWeek = lessons.filter((lesson: any) => {
    ...
    const lastWeek = lessons.filter((lesson: any) => {
      const lessonDate = new Date(lesson.createdAt);
    ...
    if (total > 0) {
      const oldestLesson = new Date(Math.min(...lessons.map((l: any) => new Date(l.createdAt).getTime())));
      ...
      const last30Count = lessons.filter((l: any) => new Date(l.createdAt) >= last30Days).length;
      const prev30Count = lessons.filter((l: any) => {
      ...
      // Domain breakdown
      const domainCounts = lessons.reduce((acc: any, lesson: any) => {
