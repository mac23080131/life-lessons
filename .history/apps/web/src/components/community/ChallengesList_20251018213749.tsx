...
import { useChallenges, useMyChallenges, useJoinChallenge } from '@/lib/hooks/useCommunity';
...
export function ChallengesList() {
  ...
  const isJoined = (challengeId: string) => {
    return myChallengesData?.challenges?.some((c: any) => c.id === challengeId);
  };
  ...
  return (
    <div className="space-y-8">
      {/* Active Challenges */}
      {myChallengesData?.challenges && myChallengesData.challenges.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {myChallengesData.challenges.map((challenge: any, index: number) => (
              <motion.div
                key={challenge.id}
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div>
                    ...
                    <p className="text-lg font-bold text-purple-600">{challenge.daysLeft || 0}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 mb-1">{t('challenges.streak')}</p>
                    <p className="text-lg font-bold text-amber-600">{challenge.streak || 0}</p>
                  </div>
                  <div>
