...
import { Save, Loader2, Check, AlertCircle, Edit, Share2 } from 'lucide-react'
...
export default function LessonDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Đã lưu thay đổi', {
        description: 'Bài học của bạn đã được cập nhật thành công',
      })
  ...
  const handleAnalyze = async () => {
    try {
      toast.promise(
        analyzeLesson.mutateAsync(resolvedParams.id),
        {
          loading: 'AI đang phân tích bài học...',
          success: (data) => {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Bài học này có thể đã bị xóa hoặc bạn không có quyền truy cập</p>
        </div>
      </div>
    )
  }
            ...
            {isEditing ? (
              ...
              <>
                <button
                  ...
                  className="px-6 py-3 rounded-xl border-2 border-border hover:bg-blue-500/10 hover:border-blue-500 transition-all flex items-center gap-2 text-blue-600 dark:text-blue-400"
                >
                  <Share2 size={18} />
                  Chia sẻ
                </button>
