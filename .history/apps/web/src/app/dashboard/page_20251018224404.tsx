        ...
        {showQuickCaptureModal && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-x-2 top-1/2 -translate-y-1/2 sm:inset-x-4 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-2xl z-50"
            >
              <div className="glass-card p-4 sm:p-6 rounded-xl sm:rounded-2xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
                {/* Header */}
                ...
                <textarea
                  ...
                  onChange={(e) => setContent(e.target.value)}
                  placeholder="Hôm nay mình học được..."
                  className="w-full h-32 px-4 py-3 border-2 border-border rounded-xl resize-none focus:ring-2 focus:ring-primary focus:border-transparent bg-background/50 backdrop-blur-sm transition-all mb-4"
                  autoFocus
                />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        value={domain}
                        onChange={(e) => setDomain(e.target.value)}
                        className="w-full px-4 py-2 border-2 border-border rounded-xl bg-background/50 backdrop-blur-sm focus:ring-2 focus:ring-primary focus:border-transparent"
                      >
                        <option value="INNER">🧠 Inner</option>
                        <option value="HEALTH">💪 Health</option>
                        <option value="RELATIONSHIP">❤️ Relationship</option>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={handleQuickCapture}
                    disabled={!content.trim() || createLesson.isPending}
                    className="flex-1 btn-modern flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {createLesson.isPending ? (
                      <>
                        <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
