  ...
  async checkAndEnqueueReminders() {
    ...
    try {
      ...
      for (const reminder of reminders) {
        ...
        const currentHourInTz = parseInt(formatInTimeZone(now, userTz, 'HH'));
        const currentMinute = parseInt(formatInTimeZone(now, userTz, 'mm'));
        ...
        if (currentHourInTz === reminder.hour && currentMinute < 5) {
          ...
          await this.reminderQueue.add(
            ...
            {
              jobId: `${reminder.id}-${formatInTimeZone(now, userTz, 'yyyy-MM-dd-HH')}`,
