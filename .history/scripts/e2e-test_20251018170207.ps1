...
$lessons = @(
    @{ content = "Hôm nay mình học được cách kiên nhẫn hơn khi đối mặt với thử thách"; domain = "INNER"; mood = 2; resonance = 3 }
    @{ content = "Tập thể dục buổi sáng giúp mình tập trung cả ngày"; domain = "HEALTH"; mood = 1; resonance = 2 }
    @{ content = "Lắng nghe chủ động giúp cải thiện mối quan hệ với đồng nghiệp"; domain = "RELATIONSHIP"; mood = 1; resonance = 3 }
    @{ content = "Lập ngân sách chi tiết giúp kiểm soát chi tiêu tốt hơn"; domain = "FINANCE"; mood = 0; resonance = 2 }
    @{ content = "Thiền định 10 phút mỗi sáng giúp mình bình tĩnh hơn"; domain = "INNER"; mood = 2; resonance = 3 }
)
...
Test-Step "11. Update Lesson (add tags and gratitude)" {
    $lessonId = $lessonIds[0]
    $body = @{
        contentRaw = "Hôm nay mình học được cách kiên nhẫn hơn và học cách thở sâu khi căng thẳng"
        domain = "INNER"
        mood = 1
        resonance = 3
        tags = @("patience", "stress-management", "e2e-test")
        gratitude = "Biết ơn về thử thách hôm nay"
    } | ConvertTo-Json
    ...
    if ($json.gratitude -ne "Biết ơn về thử thách hôm nay") {
